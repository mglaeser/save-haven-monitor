...
function build() {
  ...
  if (OUT !== __dirname) {
    ...
    // Shallow-frontend program (DR-014 §6): dist/content/ joins the published allowlist. The
    // fallback content artifact (content/fallback.json) is GENERATED by deploy.yml at deploy time
    // from the content API — never committed to the repo — so the build guarantees only the
    // directory; the deploy step writes (or preserves the previous deploy's) fallback.json there.
