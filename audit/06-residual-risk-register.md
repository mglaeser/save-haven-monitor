...
|---|---|---|---|---|---|
| **R-GATE** — **CLOSED 2026-08-28 (DR-013)** | ~~The verify gate is advisory on deploy~~ — closed: the `main` ruleset (updated 2026-08-18) requires the `gate` + `acceptance` checks, mandates PRs, and refuses force pushes/deletion; Pages `build_type=workflow` deploys only after the full gate passes (DR-006). | A-01, B-01, B-06, B-09 (re-assessed with dated history entries; A-01/B-01 FAIL→PARTIAL, counts unchanged) | The ruleset + Actions-gated deploy ARE the standing control now; `verify/tests/60-governance` keeps the workflow wired. | Ruleset drift is owner-visible in repo settings; `61-provenance`/`60-governance` red on tampering with the gate itself. | Done by `mglaeser` (ruleset 2026-08-18); recorded and evidenced in `audit/decisions/DR-013-r-gate-closure.md`. |
