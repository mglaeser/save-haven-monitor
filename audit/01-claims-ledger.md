...
| D10 | "CORS … is NOT present as of 3.1.0 and must be added … browser calls will be blocked" | INTEGRATION_NOTES | **STALE** | bubblegauge shipped CORS (DASHBOARD_FEED_SPEC v1.0 §3: "CORS already allows crash.klee.me"). **Finding: doc drift; update** |
| D11 | "Verified: zero footprint, zero network to `*.klee.me`, 5 tabs, byte-identical render" | INTEGRATION_NOTES | TRUE† | †Session-produced Playwright evidence; **but the verification itself is unrepeatable by anyone else — the harness does not live in the repo.** That gap is the subject of Phase 2 |
