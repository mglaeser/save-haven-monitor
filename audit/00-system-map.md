...
GitHub Pages from the repository root at the custom domain `https://crash.klee.me` (CNAME file).
...
|---|---|---|
| Production | GitHub Pages, branch `main`, root; custom domain `crash.klee.me` | Any push to `main` — **no admission control of any kind** |
...
| `bubblegauge.klee.me` | ONLY when `?status-api=<key>` gate active; GET-only reads | Key whitelist `^[a-z0-9-]{1,32}$`; same-registrable-domain subdomain derivation; demo mode = zero network |
...
**Production.** The site serves public traffic at `crash.klee.me`. It holds no secrets, no
